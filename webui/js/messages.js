...
export function getHandler(type) {
  switch (type) {
    ...
    case "hint":
      return drawMessageInfo;
    case "ghost_ui":
      return drawMessageGhostUI;
    case "dynamic_frame":
      return drawMessageDynamicFrame;
    default:
...
// Ghost UI message handler - Library Adapter Layer
export function drawMessageGhostUI(messageContainer, id, type, heading, content, temp, kvps) {
  try {
    const data = JSON.parse(content);
    const { library, component_id, action, config } = data;
    
    // Handle updates and destroys
    if (action === "update") {
      const component = document.getElementById(component_id);
      if (component && window.GhostUIAdapters) {
        window.GhostUIAdapters.update(library, component_id, config);
      }
      return; // Don't create new message for updates
    }
    
    if (action === "destroy") {
      const component = document.getElementById(component_id);
      if (component && window.GhostUIAdapters) {
        window.GhostUIAdapters.destroy(library, component_id);
      }
      return; // Don't create new message for destroys
    }
    
    // For create action, render the component
    let messageDiv = messageContainer.querySelector(".message");
    if (!messageDiv) {
      messageDiv = document.createElement("div");
      messageDiv.classList.add("message", "ghost-ui-message");
      messageContainer.appendChild(messageDiv);
    }
    ...
    if (heading) {
      ...
      headingElement.innerHTML = `<h4>${heading}</h4>`;
    }
    
    // Create container for the component
    let contentDiv = messageDiv.querySelector(".msg-content");
    if (!contentDiv) {
      contentDiv = document.createElement("div");
      contentDiv.classList.add("msg-content", "ghost-ui-content");
      messageDiv.appendChild(contentDiv);
    }
    
    // Create the component container
    const componentContainer = document.createElement("div");
    componentContainer.id = component_id;
    componentContainer.className = `ghost-ui-component ghost-ui-${library}`;
    componentContainer.setAttribute("data-library", library);
    contentDiv.appendChild(componentContainer);
    
    // Wait for next tick to ensure DOM is ready
    setTimeout(() => {
      // Initialize the library adapter
      if (window.GhostUIAdapters) {
        try {
          window.GhostUIAdapters.create(library, component_id, config);
        } catch (e) {
          console.error("Failed to create component:", e);
          const errorDiv = document.getElementById(component_id);
          if (errorDiv) {
            errorDiv.innerHTML = `<div class="error">Failed to create ${library} component: ${e.message}</div>`;
          }
        }
      } else {
        console.error("GhostUIAdapters not loaded");
        componentContainer.innerHTML = `<div class="error">Ghost UI Adapters not initialized</div>`;
      }
    }, 0);
    
  } catch (e) {
    console.error("Error rendering Ghost UI component:", e);
    drawMessageError(messageContainer, id, type, "Ghost UI Error", `Failed to render component: ${e.message}`, temp, kvps);
  }
}

// Dynamic Frame message handler - Universal UI component creator
export function drawMessageDynamicFrame(messageContainer, id, type, heading, content, temp, kvps) {
  // Use the same handler as Ghost UI since they share the same structure
  return drawMessageGhostUI(messageContainer, id, type, heading, content, temp, kvps);
}
