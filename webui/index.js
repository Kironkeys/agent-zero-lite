...
async function startPolling() {
  // OPTIMIZED POLLING: Reduced CPU usage while keeping chat responsive
  const shortInterval = 100;  // Fast polling when active
  const longInterval = 500;  // Slower when idle but still responsive
  const shortIntervalPeriod = 20;  // 2 seconds of fast polling
  let shortIntervalCount = 0;
  
  // Track if we're actively waiting for a response
  ...
  async function _doPoll() {
    ...
    try {
      ...
      // Check if agent is currently processing (look for pending state)
      const isPending = document.querySelector('.chat-message.pending') !== null;
      isWaitingForResponse = isPending || document.querySelector('#pause-btn')?.style.display !== 'none';
      
      if (result || isWaitingForResponse) {
        shortIntervalCount = shortIntervalPeriod; // Reset the counter when active
      }
      ...
      console.error("Error:", error);
      nextInterval = longInterval * 2; // Back off on errors
    }
...
// SSE connection for Console tab
let sseConnection = null;
let sseReconnectTimeout = null;

function initializeConsoleSSE() {
  // NEW CLEAN SSE - Proper connection to Ghost phantom console updates
  console.log('Initializing clean SSE for phantom console updates...');
  ...
  const alpineData = globalThis.Alpine ? Alpine.$data(consoleSection) : null;
  if (!alpineData) return;
  
  // Connect to the correct Ghost SSE endpoint
  try {
    sseConnection = new EventSource('/phantom_sse');
    ...
    sseConnection.onmessage = function(event) {
      try {
        ...
        console.log('[PhantomSSE] Received event:', data);
        
        // Handle chart/design creation events
        if (data.type === 'chart_created' && data.data.visualization) {
          console.log('[PhantomSSE] Chart created, refreshing workspace...');
          
          // Refresh the workspace tab to show new chart
          if (window.phantomConsole && window.phantomConsole.refreshWorkspace) {
            window.phantomConsole.refreshWorkspace();
          }
        } else if (data.type === 'design_created' && data.data.design) {
          console.log('[PhantomSSE] Design created, refreshing designs...');
          
          // Refresh the designs tab to show new design  
          if (window.phantomConsole && window.phantomConsole.refreshDesigns) {
    ...
    sseConnection.onerror = function(error) {
      ...
      // Smart reconnect - only if not manually disabled
      if (!sseReconnectTimeout && !window.sseDisabled) {
        sseReconnectTimeout = setTimeout(() => {
          console.log('[PhantomSSE] Attempting reconnect...');
          initializeConsoleSSE();
        }, 10000); // Wait 10 seconds before reconnect
      }
    };
  } catch (e) {
    console.log('[PhantomSSE] Connection failed:', e);
  }
}

// Make SSE function globally available
globalThis.initializeConsoleSSE = initializeConsoleSSE;

// Add connectToSSE function for manual reconnection
globalThis.connectToSSE = function() {
  console.log('[PhantomSSE] Manual reconnection requested');
  
  if (sseConnection) {
    sseConnection.close();
  }
  window.sseDisabled = false; // Enable SSE
  initializeConsoleSSE();
};

// Toggle Phantom Console panel
globalThis.togglePhantomConsole = function() {
  console.log('[togglePhantomConsole] Function called');
  
  const panel = document.getElementById('phantom-console');
  const mainContent = document.querySelector('.chat-container') || document.querySelector('.container') || document.querySelector('main');
  
  console.log('[togglePhantomConsole] Panel found:', !!panel);
  console.log('[togglePhantomConsole] Main content found:', !!mainContent);
  
  if (panel) {
    const isCurrentlyOpen = panel.style.right === '0px';
    console.log('[togglePhantomConsole] Panel currently open:', isCurrentlyOpen);
    
    if (isCurrentlyOpen) {
      // Close panel
      panel.style.right = `-${panel.style.width || '600px'}`;
      if (mainContent) {
        mainContent.style.marginRight = '0px';
      }
      
      // Remove body class
      document.body.classList.remove('phantom-console-open');
      
      console.log('[Phantom Console] Panel closed');
    } else {
      // Open panel
      panel.style.right = '0';
      const panelWidth = parseInt(panel.style.width) || 600;
      if (mainContent) {
        mainContent.style.marginRight = panelWidth + 'px';
      }
      console.log('[Phantom Console] Panel opened');
      
      // Add body class for styling
      document.body.classList.add('phantom-console-open');
      
      // Load visualizations and fetch data when opening
      setTimeout(() => {
        if (window.loadInitialVisualizations) {
          console.log('[Phantom Console] Loading initial visualizations...');
          window.loadInitialVisualizations();
        }
        if (window.fetchPhantomConsoleData) {
          window.fetchPhantomConsoleData();
        }
      }, 100);
    }
  } else {
    console.warn('[Phantom Console] Panel not found! Trying to initialize...');
    
    // Try to initialize if it hasn't been done yet
    if (window.initializePhantomConsole) {
      console.log('[Phantom Console] Running initialization...');
      window.initializePhantomConsole();
    } else {
      console.error('[Phantom Console] No initialization function available');
    }
  }
};
