...
class Log:
    ...
    def __init__(self):
        ...
        self._suppressed_updates = 0
        
        # Configuration: Allow max 20 updates per second with burst of 50
        self.max_updates_per_second = 20
        self.burst_size = 50
    ...
    def log(
        ...
        **kwargs,
        ...
        # Rate limiting check (skip for errors and critical messages)
        if type not in ["error", "warning", "response", "user"]:
            if not self._should_allow_update():
                self._suppressed_updates += 1
                # Return a dummy item that won't be sent to frontend
                return LogItem(
                    log=self,
                    no=-1,
                    type=type,
    ...
    def _should_allow_update(self) -> bool:
        """Check if an update should be allowed based on rate limiting."""
        with self._rate_limit_lock:
            current_time = time.time()
            
            # Reset counter if we're in a new time window
            if current_time - self._last_update_time > self.time_window:
                self._update_count = 0
                self._last_update_time = current_time
                
                # Log suppressed updates if any
                if self._suppressed_updates > 0:
                    print(f"[Rate Limiter] Suppressed {self._suppressed_updates} updates in the last window")
                    self._suppressed_updates = 0
            
            # Check if we're within burst limit
            if self._update_count < self.burst_size:
                self._update_count += 1
                return True
            
            # Check if we're within sustained rate
            elapsed = current_time - self._last_update_time
            allowed_updates = int(elapsed * self.max_updates_per_second)
            
            if self._update_count < allowed_updates:
                self._update_count += 1
                return True
