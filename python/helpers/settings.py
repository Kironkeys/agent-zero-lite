...
def convert_out(settings: Settings) -> SettingsOutput:
    ...
    browser_model_fields.append(
        {
            ...
            "description": "Custom HTTP headers for browser sessions. Useful for authentication tokens, custom User-Agent strings, etc. Format is KEY=VALUE on individual lines, just like .env file.",
            "type": "textarea",
    ...
    # LiteLLM Global Configuration section
    litellm_fields: list[SettingsField] = []

    litellm_fields.append(
        {
            "id": "litellm_global_kwargs",
            "title": "LiteLLM Global Configuration",
            "description": "Global parameters applied to all LiteLLM model calls. These settings help with provider compatibility and parameter normalization. Format is KEY=VALUE on individual lines, just like .env file.",
            "type": "textarea",
    ...
    litellm_section: SettingsSection = {
        "id": "litellm",
        "title": "LiteLLM Global Config",
        "description": "Global configuration parameters applied to all LiteLLM model calls across all providers.",
...
def get_runtime_config(set: Settings):
    if runtime.is_dockerized():
        return {
            ...
            "a2a_server_enabled": set["a2a_server_enabled"],  # Add A2A config
        }
    else:
        host = set["rfc_url"]
        if "//" in host:
