...
class A2aSubordinate(Tool):
    ...
    async def execute(
        ...
        **kwargs
        ...
        try:
            # Validate A2A is enabled (check both possible config names for compatibility)
            if not getattr(self.agent.config, 'a2a_enabled', False) and not getattr(self.agent.config, 'a2a_server_enabled', False):
                return Response(
                    message="Error: A2A protocol must be enabled to use A2A subordinates. Enable a2a_server_enabled in settings.",
                    break_loop=False
                )
            ...
            shared_context = {
                ...
                "parent_context_id": self.agent.context.id,  # Use 'id' not 'context_id'
