...
thread_lock = threading.Lock()  # Global thread lock for API handlers
...
# SSE endpoints removed - were causing CPU overload with constant reconnection attempts


# Hume EVI Direct endpoint for speech-to-speech
@webapp.route('/hume_evi_direct', methods=['POST', 'GET', 'OPTIONS'])
def hume_evi_direct():
    """Direct Hume EVI connection handler"""
    import asyncio
    try:
        from python.api.hume_evi_direct import HumeEviDirect
        ...
        # Process request with async handler
        handler = HumeEviDirect(webapp, thread_lock)
        # Run async method in sync context
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            result = loop.run_until_complete(handler.process(data, request))
        finally:
            loop.close()
        ...
        PrintStyle.error(f"Hume EVI Direct error: {e}")
        return jsonify({"error": str(e), "success": False}), 500


@webapp.route('/api/elevenlabs/credentials', methods=['GET'])
def get_elevenlabs_credentials():
    ...
    try:
        ...
        agent_id = None
        agent_file = get_abs_path('.elevenlabs_agent_id')
...
@webapp.route('/api/elevenlabs/save-agent', methods=['POST'])
def save_elevenlabs_agent():
    """Save ElevenLabs agent ID for persistence"""
    try:
        data = request.get_json()
